...
def app():
    mat = scipy.loadmat('data/grupoDados2.mat')
    ...
    #melhor valor de k = 1, precisao 98.3%
    #automatizei a busca pelo melhor valor de k até o 29, e encontrei o k1 como o melhor

def obterMelhorK(dadosTrain, rotuloTrain, dadosTest, rotuloTest):
    k = 1
    precisao = 0
    while k < 31:
        classes = meuKnn(dadosTrain, rotuloTrain, dadosTest, k)
        n_precisao = acurracia(classes, rotuloTest)
        if n_precisao > precisao:
            precisao = n_precisao
            k_best = k
        k += 2
    return [k_best, precisao]
...
def visualizaPontos(dados, rotulos, d1, d2, titulo):
    fig, ax = plt.subplots() 
    plt.title(titulo)
